...
with mp_holistic.Holistic(
    ...
    while cap.isOpened():
        ...
        if not success:
            ...
            pose_df.to_excel(save_path+file_name+"pose.xlsx")
    ...
    #    if(results.pose_landmarks is not None and results.left_hand_landmarks is not None and results.right_hand_landmarks is not None):
        if results.pose_landmarks: 
            ...
            for i in range(len(pose_tubuh)):
                ...
                data_tubuh.update(
                {pose_tubuh[i]+" x" : results.pose_landmarks.landmark[i].x,pose_tubuh[i]+" y" : results.pose_landmarks.landmark[i].y,pose_tubuh[i]+" z" : results.pose_landmarks.landmark[i].z,pose_tubuh[i]+" visiblility" : results.pose_landmarks.landmark[i].visibility,}
                )
                print(results)
            ...
            if results.right_hand_landmarks:
                ...
                for i in range(len(pose_tangan)):
                    ...
                    {pose_tangan[i]+" x" : results.right_hand_landmarks.landmark[i].x,pose_tangan[i]+" y" : results.right_hand_landmarks.landmark[i].y,pose_tangan[i]+" z" : results.right_hand_landmarks.landmark[i].z,pose_tangan[i]+" visiblility" : results.right_hand_landmarks.landmark[i].visibility,}
                    )
            ...
            if results.left_hand_landmarks:
                ...
                for i in range(len(pose_tangan_2)):
                    ...
                    {pose_tangan_2[i]+" x" : results.left_hand_landmarks.landmark[i].x,pose_tangan_2[i]+" y" : results.left_hand_landmarks.landmark[i].y,pose_tangan_2[i]+" z" : results.left_hand_landmarks.landmark[i].z,pose_tangan_2[i]+" visiblility" : results.left_hand_landmarks.landmark[i].visibility,}
                    )
            ...
            for i in range(len(pose_tangan)):
                ...
                results.right_hand_landmarks.landmark[i].y = results.right_hand_landmarks.landmark[i].y * image.shape[1]
                data_tangan_kanan.update(
                {pose_tangan[i]+" x" : results.right_hand_landmarks.landmark[i].x,pose_tangan[i]+" y" : results.right_hand_landmarks.landmark[i].y,pose_tangan[i]+" z" : results.right_hand_landmarks.landmark[i].z,pose_tangan[i]+" visiblility" : results.pose_landright_hand_landmarksmarks.landmark[i].visibility,}
                )
            right_hand_data.append(data_tangan_kanan)
        ...
        cv2.imshow('MediaPipe Pose',cv2.resize(image,(0,0),fx=0.5,fy=0.5, interpolation=cv2.INTER_AREA) )
